...
| Tool deep links | Shareable links such as `/tools#command-safety` and `/en/tools#command-safety` keep opening | Fix the interactive tool hash map or documentation |
...
`release:check` verifies key HTML files, `skills.json`, `sitemap.xml`, `robots.txt`, the interactive tool deep-link contract, and the `web/out/api/search/{locale}` search index shards.
...
```bash
for path in / /en /ai/skills /en/ai/skills /scenarios /en/scenarios /hardware /en/hardware /versions /en/versions /tools /en/tools '/tools#command-safety' '/en/tools#command-safety'; do
