...
| 工具深链 | `/tools#command-safety`、`/en/tools#command-safety` 等分享链接保持可打开 | 修复交互工具 hash 映射或文档说明 |
...
`release:check` 会检查 `web/out` 中的关键 HTML、`skills.json`、`sitemap.xml`、`robots.txt`、交互工具深链契约，以及 `web/out/api/search/{locale}` 搜索索引分片。
