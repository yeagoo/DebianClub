...
本页按 **2026-09-08** 的 Debian 官方发布信息整理。Debian 版本状态会随点更新、安全支持切换和 LTS 公告变化；执行升级前，请再次查看 [Debian 发布信息](https://www.debian.org/releases/) 与 [Debian LTS](https://wiki.debian.org/LTS)。
...
| 仍在 Debian 11 | **尽快升级到 Debian 12，再升级 Debian 13** | Bullseye LTS 已于 2026-08-31 结束，仅部分包由第三方 ELTS 支持；不适合继续新增部署 |
...
| Debian 12 (Bookworm) | oldstable | 暂时不能升级的旧系统 | 常规安全支持已结束，已转入 LTS | 制定升级窗口 |
| Debian 11 (Bullseye) | LTS 已结束（ELTS） | 仅限遗留系统 | 第三方 ELTS（部分包） | 迁移、隔离或下线 |
...
| Debian 11 (Bullseye) | 2021-08-14 | 已结束 | 2026-08-31（已结束） | 迁移、ELTS 或下线 |
| Debian 10 (Buster) | 2019-07-06 | 已结束 | 2024-06-30 | 不应继续暴露在公网 |

Debian 11 的 LTS 已于 **2026-08-31** 结束；Debian 不再提供安全更新，部分包由第三方 ELTS 支持至 **2031-06-30**。

截至 **2026-09-08**，Debian 13 的最新点更新为 **13.6**（2026-07-11），Debian 12 为 **12.15**（2026-07-11）；Debian 13.7 计划于 2026-09-12 发布。
...
上表是各版本发布时的基线版本。想查看某个包在 Debian 及其他主流发行版中的**当前实际版本**、依赖关系和 CVE 状态，可以使用 [pkgseek](https://pkgseek.com/) 的跨发行版查询（下方矩阵实时加载自 pkgseek）：

<iframe src="https://pkgseek.com/embed/packages/nodejs?compact=true&locale=zh-CN" title="nodejs 版本矩阵 · PkgSeek" loading="lazy" width="100%" height="320" style={{border: 0}} referrerPolicy="no-referrer-when-downgrade"></iframe>

<iframe src="https://pkgseek.com/embed/packages/python3.13?compact=true&locale=zh-CN" title="python3.13 版本矩阵 · PkgSeek" loading="lazy" width="100%" height="320" style={{border: 0}} referrerPolicy="no-referrer-when-downgrade"></iframe>

查询任意软件包：[pkgseek.com](https://pkgseek.com/)；包页面地址格式为 `https://pkgseek.com/packages/<包名>`。
...
| Debian 12 | 规划升级 Debian 13 | 常规安全支持已于 2026-07-11 结束，当前处于 LTS，应规划升级 |
| Debian 11 | 先升级 Debian 12，再升级 Debian 13；无法升级则评估 ELTS 或下线 | LTS 已于 2026-08-31 结束，避免跨多个 stable 版本直接升级 |
